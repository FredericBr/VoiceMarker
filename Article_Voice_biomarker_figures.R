...
##Fig. 3c. Classification of autistic children with respect to a diverse control population
...
Best_AUC=c("avg_F1","shimmer","shimmer_k")
Combi=combinations(n=3, r=2, v=Best_AUC)
...
p2=ggplot(Databis, aes_string(Combi[2,1], Combi[2,2],color = "Group")) +
        geom_point(shape = 16, size = 3) +ylab("k(Shimmer)")+xlab("F1") +
        #theme(plot.margin = unit(c(6,0,6,0), "pt")) + 
        scale_color_manual(values=c("#FF4E28", "#28FFAE", "#0CC17D", "#20976B")) 
p3=ggplot(Databis, aes_string(Combi[3,1], Combi[3,2],color = "Group")) +
        geom_point(shape = 16, size = 3) +ylab("k(Shimmer)")+xlab("Shimmer") +
        #theme(plot.margin = unit(c(6,0,6,0), "pt")) + 
        scale_color_manual(values=c("#FF4E28", "#28FFAE", "#0CC17D", "#20976B")) 
...
#windows(12,5)
tiff("Figure2_Ex.Model4_color.tiff", width= 2604, height= 1000, units="px", res=300)
p
dev.off()
...
df_TSA=xdf[xdf$Group=="ASD",];df_TSA<- df_TSA[,-6] #remove Group
df_TD=xdf[xdf$Group=="TD",];df_TD<- df_TD[,-6] #remove Group

averag_ASD <- data.frame(avg_F1 = mean(df_TSA$avg_F1),
                         mean_hnr = mean(df_TSA$mean_hnr),
                         shimmer = mean(df_TSA$shimmer),
                         shimmer_k = mean(df_TSA$shimmer_k),
...
averag_TD <- data.frame(avg_F1 = mean(df_TD$avg_F1),
                         ...
                         shimmer_k = mean(df_TD$shimmer_k),
...
max_min <- data.frame(
        avg_F1 = c(-4, 4), mean_hnr = c(-4, 4), shimmer = c(-4, 4),
        shimmer_k = c(-4, 4), jitter_s = c(-4, 4))
...
setnames(df_TSA, old = c("avg_F1", "mean_hnr", "shimmer", "shimmer_k", "jitter_s"),
        new = c("F1", "HNR", "Shimmer","k(Shimmer)","s(Jitter)"))
setnames(df_TD, old = c("avg_F1", "mean_hnr", "shimmer", "shimmer_k", "jitter_s"),
         new = c("F1", "HNR", "Shimmer","k(Shimmer)","s(Jitter)"))
...
tiff("Figure2c.tiff", width= 2604, height= 1000, units="px", res=300)
